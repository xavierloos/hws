...
export const GET = async (req: Request, { params }: any) => {
	try {
		...
		// blog.banner.src = await getTemporaryUrl('files', blog.banner);
		blog.user.src = await getTemporaryUrl(`${blog.user.image}/${blog.user.id}`);

		if (!blog) return NextResponse.json({ message: 'Not found' }, { status: 500 });
...
export const POST = async (req: Request, { params }: any) => {
	try {
		...
		if (!blog) return NextResponse.json({ message: 'Blog does not exist' }, { status: 500 });
		...
		return NextResponse.json(
			{
				message: user ? 'Comment added' : 'Thank you for your comment, and email has been sent to verify',
				type: 'success',
			},
			{ status: 200 }
		);
