				...
				return (
					<User
						avatarProps={{
							...
							size: 'sm',
							radius: 'full',
							src: cellValue.src || i.creator.src || i.modifier.src,
						}}
						description={
							<span className='truncate text-ellipsis line-clamp-1 text-tiny'>
								@{cellValue.username || i.creator.username || i.modifier.username}
							</span>
						}
						name={
							<span className={`text-ellipsis overflow-hidden break-words line-clamp-1 text-sm`}>
								{cellValue.name || i.creator.name || i.modifier.name}
							</span>
						}
					/>
				);
		...
		return (
			<div className='flex flex-col gap-4'>
				...
				<div className='flex justify-between items-center'>
					<span className='text-default-400 text-tiny'>Total {data.length} blogs</span>
		...
		<>
			...
			>
				<ModalContent>
					...
					<EditBlog item={editItem} onSubmit={onSubmitEdit} isSaving={isSavingEdit} />
				</ModalContent>
			</Modal>
			{/* EDIT BLOG ENDS */}
		</>
