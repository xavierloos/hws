          ...
          <div className="flex justify-between w-full">
            <div>
              <div className="flex items-center">
                ...
                <h1 className=" text-large font-semibold ">{data?.name}</h1>
              </div>
              <div className=" text-muted-foreground text-tiny">
                Due {format(data?.dueDate)} • On{" "}
                {dateFormat(data?.dueDate, "ddd dd/mmm/yy - HH:MM")}
              </div>
            </div>
            <Button
              onClick={handleTaskPreviewer}
              size="md"
              isIconOnly
              color="default"
              variant="light"
                  ...
                  if (i === val.id) {
                    return (
                      <Tooltip
                        ...
                        content={val.name}
                      >
                        <Avatar isBordered size="sm" src={val.image} />
                      </Tooltip>
                    );
                  }
                })
              ...
              <Popover showArrow placement="bottom">
                <PopoverTrigger>
                  <Avatar
                    ...
                    showFallback
                    fallback={
                      <PlusIcon className="text-primary" fill="currentColor" />
                    }
                  />
                </PopoverTrigger>
                <PopoverContent className="p-1 min-w-[300px] border-none ">
                  <Input
                    size="sm"
                    type="text"
                    label="Search member"
                  ...
                  <Button
                    ...
                    color="primary"
                    variant="solid"
                    className="p-0 rounded-md mx-2 mb-1 w-full"
          ...
          <Accordion className="p-0 border-1 px-2 rounded-md">
            ...
            >
              <div>
                ...
                >
                  <div className="flex">
                    <Avatar src={data?.user?.image} className="w-6 h-6 me-2" />
                    <Textarea
                      // label="Comment"
                      placeholder="What are you thinking?"
                      size="sm"
                      className="w-full border-1 rounded-md text-tiny"
                      color="default"
                      isRequired
                      minRows={1}
                      ...
                      <div>
                        <div
                          className={`w-fit h-auto px-2 py-1 rounded-sm text-xs text-ellipsis text-content5 font-light  overflow-hidden break-words m-auto ${
                            item.user?.id == user?.id
                              ? "text-start ms-0  bg-primary text-secondary rounded-tl-none"
                              : "text-end me-0  bg-secondary  rounded-tr-none"
                          }`}
                        >
                          {item?.comment}
                        </div>
                        ...
                        >
                          <span>
                            {item.user.id === user.id ? "Me" : item?.user?.name}
                          </span>
                          <span>
                            {item?.attachments &&
