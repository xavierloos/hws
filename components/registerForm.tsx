...
export const RegisterForm = () => {
	...
	const togglePassword2Visibility = () => setPassword2Visible(!password2Visible);
			...
			<Card className='border-none bg-content1 max-w-md w-full rounded-md shadow-md'>
				<CardBody className='grid  grid-cols-1  gap-2 items-center justify-center py-4 px-8'>
					...
					{!error && !loading && !success && !message ? (
						<>
							<form className='grid gap-4 pb-4' onSubmit={(e) => onSubmit(e, inputs)}>
								<Input
									size='sm'
									isRequired
									radius='none'
						...
						<Title text={error ? 'Register invitation' : ''} subtext={error} className='items-start mb-4' />
					)}
				</CardBody>
			</Card>
		</>
