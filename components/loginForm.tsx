...
import { EyeClosedIcon, EyeOpenIcon, GitHubLogoIcon } from '@radix-ui/react-icons';
...
export const LoginForm = () => {
	...
	const urlError =
		searchParams.get('error') === 'OAuthAccountNotLinked' ? 'Account already used with another provider' : '';
	...
	const onForgotPasword = (values: any) => {
		startTransition(() => {
			if (inputs.email == undefined || inputs.email === '' || inputs.email === null) {
				toast.warning('Pleasse provide an email');
			} else {
				toast.warning(`Are you sure you want to reset password for: ${values}?`, {
					action: {
						label: 'YES',
						onClick: async () => {
							try {
				...
				<form className='grid gap-4' onSubmit={(e) => onSubmit(e, inputs)}>
					{showOTP ? (
						<div className='w-full flex justify-center'>
							<InputOTP required maxLength={6} onChange={(e) => setInputs({ ...inputs, code: e })}>
								<InputOTPGroup>
